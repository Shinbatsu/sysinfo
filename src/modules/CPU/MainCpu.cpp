...
MainCpu::MainCpu(void) :
    _name("CPU"),
    _min(0.0f),
    _max(100.0f),
    _graphs(std::map<std::string, std::deque<float>>()),
    _data(std::map<std::string, std::string>()) {
    this->update();
}

MainCpu::MainCpu(MainCpu const& src) { *this = src; }
...
void MainCpu::setCPUsLoad() {
    ...
    if (host_processor_info(mach_host_self(), PROCESSOR_CPU_LOAD_INFO, &cpuCount,
                            reinterpret_cast<processor_info_array_t*>(&cpuInfo), &nbInfo)
        != KERN_SUCCESS)
    ...
    for (natural_t i = 0; i < cpuCount; i++)
    {
        ...
        dequeUpdate(ss.str(), calculateCPULoad(system + user, _oldCpusWorkTicks[i],
                                               system + user + idle, _oldCpusTotalTicks[i]));
    ...
    dequeUpdate("CPU Total Usage (%)",
                calculateCPULoad(newWork, _oldWorkTicks, newTotal, _oldTotalTicks));

    _oldTotalTicks = newTotal;
    _oldWorkTicks  = newWork;
}

float MainCpu::calculateCPULoad(size_t newWorkTicks,
                                size_t oldWorkTicks,
                                size_t newTotalTicks,
                                size_t oldTotalTicks) {
...
const float& MainCpu::getGraphMin(void) const { return this->_min; }

const float& MainCpu::getGraphMax(void) const { return this->_max; }

const std::map<std::string, std::deque<float>>& MainCpu::getGraphs(void) const {
    return this->_graphs;
}

const std::map<std::string, std::string>& MainCpu::getData(void) const { return this->_data; }

const std::string& MainCpu::getName(void) const { return this->_name; }

void MainCpu::setName(std::string name) { this->_name = name; }
